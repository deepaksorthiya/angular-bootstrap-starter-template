...
export class ThemePickerComponent {
  ...
  current = computed(
    () => this.themes().find((t) => t.id === this.preferred()) || this.themes()[0],
  );
  systemTheme = signal(PREFERS_COLOR_SCHEME_DARK.matches ? 'dark' : 'light');
  dataBsTheme = computed(() =>
    this.current().id === 'auto' ? this.systemTheme()! : this.current().id,
  );
